...
def convert_path(path):
    ...
    HDR_path = HDR_path.replace('_short', '_medium')
    HDR_path = HDR_path.replace('short/', '')
    align_path = HDR_path.replace('medium.png', 'exposures.npy')
...
class Create_Dataset(keras.utils.Sequence):
...
def psnr_tanh_norm_mu_tonemap(hdr_nonlinear_ref, hdr_nonlinear_res, percentile=99, gamma=2.24):
    ...
    hdr_nonlinear_ref = tf.math.log(
        tf.cast(hdr_nonlinear_ref, tf.float64) / (1 - tf.cast(hdr_nonlinear_ref, tf.float64) + .000000000000001))
    hdr_nonlinear_ref = tf.clip_by_value(hdr_nonlinear_ref, 0, tf.math.reduce_max(hdr_nonlinear_ref))
    hdr_nonlinear_res = tf.math.log(
        tf.cast(hdr_nonlinear_res, tf.float64) / (1 - tf.cast(hdr_nonlinear_res, tf.float64 + .000000000000001)))
